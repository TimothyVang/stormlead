...
def upgrade() -> None:
    if not _table_exists("budget_action_logs"):
        op.create_table(
            "budget_action_logs",
    ...
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_approval_required"):
        op.create_index(
            "ix_budget_action_logs_approval_required",
            "budget_action_logs",
            ["approval_required"],
        )
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_campaign_created"):
        op.create_index(
            "ix_budget_action_logs_campaign_created",
            "budget_action_logs",
            ["campaign_id", sa.text("created_at DESC")],
        )
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_action"):
        op.create_index("ix_budget_action_logs_action", "budget_action_logs", ["action"])
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_created_at"):
        op.create_index("ix_budget_action_logs_created_at", "budget_action_logs", ["created_at"])
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_external_campaign_id"):
        op.create_index(
            "ix_budget_action_logs_external_campaign_id",
            "budget_action_logs",
            ["external_campaign_id"],
        )
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_platform"):
        op.create_index("ix_budget_action_logs_platform", "budget_action_logs", ["platform"])
    if not _index_exists("budget_action_logs", "ix_budget_action_logs_status"):
        op.create_index("ix_budget_action_logs_status", "budget_action_logs", ["status"])
    if not _index_exists("budget_action_logs", "uq_budget_action_logs_idempotency_key"):
        op.create_index(
