...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('pending', 'active', 'disabled')", name="ck_payment_customers_status"
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("monthly_cap_cents >= 0", name="ck_wallet_autorefill_rules_monthly_cap"),
        sa.ForeignKeyConstraint(["buyer_id"], ["buyers.id"]),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
    ...
    op.create_index("ix_wallet_autorefill_rules_buyer_id", "wallet_autorefill_rules", ["buyer_id"])
    op.create_index(
        "ix_wallet_autorefill_rules_created_at", "wallet_autorefill_rules", ["created_at"]
    )
    op.create_index("ix_wallet_autorefill_rules_provider", "wallet_autorefill_rules", ["provider"])
