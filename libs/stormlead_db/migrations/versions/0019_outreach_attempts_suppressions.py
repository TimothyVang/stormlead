...
def upgrade() -> None:
    if not _table_exists("outreach_attempts"):
        op.create_table(
            "outreach_attempts",
    ...
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_buyer_id"):
        op.create_index("ix_outreach_attempts_buyer_id", "outreach_attempts", ["buyer_id"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_channel"):
        op.create_index("ix_outreach_attempts_channel", "outreach_attempts", ["channel"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_created_at"):
        op.create_index("ix_outreach_attempts_created_at", "outreach_attempts", ["created_at"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_direction"):
        op.create_index("ix_outreach_attempts_direction", "outreach_attempts", ["direction"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_lead_id"):
        op.create_index("ix_outreach_attempts_lead_id", "outreach_attempts", ["lead_id"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_lead_created_at"):
        op.create_index(
            "ix_outreach_attempts_lead_created_at",
            "outreach_attempts",
            ["lead_id", sa.text("created_at DESC")],
        )
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_provider"):
        op.create_index("ix_outreach_attempts_provider", "outreach_attempts", ["provider"])
    if not _index_exists("outreach_attempts", "ix_outreach_attempts_status"):
        op.create_index("ix_outreach_attempts_status", "outreach_attempts", ["status"])
    if not _index_exists("outreach_attempts", "uq_outreach_attempts_provider_message"):
        op.create_index(
    ...
    if not _index_exists("outreach_attempts", "uq_outreach_attempts_idempotency_key"):
        op.create_index(
    ...
    if not _table_exists("channel_suppressions"):
        op.create_table(
    ...
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_channel"):
        op.create_index("ix_channel_suppressions_channel", "channel_suppressions", ["channel"])
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_created_at"):
        op.create_index(
            "ix_channel_suppressions_created_at", "channel_suppressions", ["created_at"]
        )
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_email"):
        op.create_index("ix_channel_suppressions_email", "channel_suppressions", ["email"])
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_phone_e164"):
        op.create_index(
            "ix_channel_suppressions_phone_e164", "channel_suppressions", ["phone_e164"]
        )
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_reason"):
        op.create_index("ix_channel_suppressions_reason", "channel_suppressions", ["reason"])
    if not _index_exists("channel_suppressions", "ix_channel_suppressions_status"):
        op.create_index("ix_channel_suppressions_status", "channel_suppressions", ["status"])
    if not _index_exists("channel_suppressions", "uq_channel_suppressions_active_email"):
        op.create_index(
    ...
    if not _index_exists("channel_suppressions", "uq_channel_suppressions_active_phone"):
        op.create_index(
    ...
    if not _index_exists("channel_suppressions", "uq_channel_suppressions_provider_event"):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "source_provider IS NOT NULL AND external_event_id IS NOT NULL"
            ),
        )
