...
claude research artifacts that informed the scaffold's choices. preserved verbatim — what was true at audit time. when the scaffold diverges from a recommendation, that divergence is intentional (see notes below).
...
## latent risks called out in `forkable-stack.md`
